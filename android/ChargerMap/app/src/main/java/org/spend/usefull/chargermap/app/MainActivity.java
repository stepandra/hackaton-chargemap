...
public class MainActivity extends Activity {
    ...
    public void onBackPressed() {
        WelcomeFragment_ myFragment = (WelcomeFragment_) getFragmentManager().findFragmentByTag(getClass().getName());
        if (myFragment == null) {
            FragmentTransaction fragmentTransaction = getFragmentManager().beginTransaction();
            fragmentTransaction.setCustomAnimations(R.anim.comming_in_from_down, R.anim.comming_out_to_bottom);
            fragmentTransaction.replace(android.R.id.content, new WelcomeFragment_(), getClass().getName()).commit();
